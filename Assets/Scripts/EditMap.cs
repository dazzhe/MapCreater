...
public class EditMap : MonoBehaviour {
	...
	Target SetTarget()
	{
		...
		RaycastHit hit;
		bool b = Physics.Raycast (ray, out hit, maxrange);
	...
	[PunRPC]
	void ApplyChangesRPC(Vector3 pos, int type, int color, float[] r, Vector3 rot)
	{
		GameObject cube;
		switch (type) {
		case 1:
	...
	void ShowIllusionSub(Vector3 pos, int type)
	{
		float[] r = {es.w, es.d, es.h};
		Vector3 rot = new Vector3(br.x*90, br.y*90, br.z*90);
	...
	Vector3 CalcPos(Vector3 pos, float[] r, Quaternion rot)
	{
		Vector3 spawnPos;
	...
	Vector3 CalcSize(float[] r)
	{
		return new Vector3 (size * r[0], size * r[1], size * r[2]);
	}

	void DeleteCube(Target target)
	{
	...
	[PunRPC]
	void DeleteCubeRPC(Vector3 from, Vector3 to)
	{
		Ray ray = new Ray (from, (to-from).normalized);
		RaycastHit hit;
		if (Physics.Raycast (ray, out hit, maxrange) && hit.collider.gameObject.tag != "Untagged") {
	...
	bool IsTargetValid(Vector3 targetPos)
	{
